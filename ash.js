...
// Extract Ashby timestamp from HTML: jsonLd.datePosted or meta article:published_time
async function fetchAshbyTimestamp(orgName, jobId) {
  ...
  try {
    ...
    console.error(`Error fetching Ashby timestamp for ${orgName}/${jobId}: ${err.message}`);
    return null;
  }
}
...
// Pick timestamp for Greenhouse job
...
async function main() {
  ...
  for (let i = 0; i < ashbyOrgArray.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async (org) => {
      ...
      for (const jobBatch of jobBatches) {
        const jobPromises = jobBatch.map(async (job) => {
          const [detail, timestamp] = await Promise.all([
            fetchAshbyJobDetail(org, job.id),
